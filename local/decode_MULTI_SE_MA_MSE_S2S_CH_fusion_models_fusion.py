...
from utils_s2s import average_checkpoints
...
def main(args):
    ...
    #from model_S2S import MULTI_MAM_SE_S2S_model
    #from model_S2S_weight_input import MULTI_MAM_SE_S2S_model
    ...
    try:
        if args.model_config != "N":
            ...
            configs = torch.load(args.model_path + '2')["configs"]
    ...
    nnet = average_checkpoints(nnet, args.model_path, args.epochs)
    # nnet = nnet.cuda()
    ...
    #print("label_init.frame_label.keys() ", label_init.frame_label.keys())
    ...
    # print(session2CH)
    # print(session2CH.keys())
    # for session_root in session2CH: 
    #     print(len(session2CH[session_root]), session_root)
    # exit()
            ...
            with torch.no_grad():
                for m in feature:
                    ...
                    if (i == args.batch_size) or (len(feature) == cur_utt): #攒够一个batch
                        ...
                        #print(torch.sum(ypreds > 0.5), ypreds.shape)
                        if isinstance(ypreds, (list, tuple)):
                            ypreds = ypreds[0]
                        ypreds = ypreds[:, :num_speaker, :]
                    
                        #print("ypreds", ypreds.shape, ypreds.transpose(0, 1).shape)
                        ypreds = Sigmoid(ypreds).transpose(0, 1).reshape(num_speaker, -1).detach().cpu().numpy()  #B, N, T -> N -> N, (B,T) ? need to check
                        # print(ypreds.shape)
                        # print(np.sum(ypreds > 0.5))
                        # exit()
                        cur_frame = 0
                        for n in nframes:
                            #print(n)
...
def make_argparse():
    ...
    parser.add_argument('--model_path', metavar='PATH', required=True,
                        help='model_path.')  
    parser.add_argument('--epochs', metavar='AVERAGE EPOCHS', required=True,
                        help='(a,b],(c,d].')  
