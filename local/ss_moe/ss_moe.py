...
import sys
sys.path.insert(0, '/train33/sppro/permanent/gbyang/code/NSD-MS2S/local_gb/soft_moe')
from einops.einops import rearrange, reduce
from einops.packing import pack, unpack
from einops.layers.torch import Rearrange

sys.path.insert(0, '/train33/sppro/permanent/gbyang/code/NSD-MS2S/local_gb/')
# from model_S2S_weight_input_DIM_MOE import MultiheadAttention
import pdb
try:
...
class Share_Expert(nn.Module):
    def __init__(self, dim, mult = 2,
                 **kwargs):
        super(Share_Expert, self).__init__(**kwargs)
...
class out_logit_transform(Module):
    def __init__(self, dim, num_heads=1, num_features=4):
        super().__init__()
        #b n (e s)
        self.attention = torch.nn.MultiheadAttention(embed_dim=dim, num_heads=num_heads, batch_first=True)
        self.inst_norm1d = torch.nn.InstanceNorm1d(num_features=num_features, affine=True)
        self.out = nn.Linear(dim, dim)
    def forward(self, logit):

        combine_weights, _ = self.attention(logit, logit, logit)
        combine_weights = self.inst_norm1d(combine_weights)
        combine_weights = self.out(combine_weights)
        combine_weights = combine_weights.softmax(dim=-1)
...
class SS_MoE(Module):
    def __init__(
        ...
        *,
        ...
        num_slots = default(num_slots, seq_len // num_experts)
        # print(num_slots, seq_len, num_experts, 'SS_MoE', "geglu", geglu)
        norm_klass = LayerNorm if use_layernorm else RMSNorm
        self.norm = norm_klass(dim)
        self.slot_norm = norm_klass(dim)
        self.slot_embeds = nn.Parameter(torch.randn(num_experts, num_slots, dim))
        self.out_logit_transform = out_logit_transform(num_experts*num_slots, num_features=seq_len)
        expert_klass = GLUFeedForward if geglu else FeedForward
        self.share_expert = Share_Expert(dim)
        self.experts = Experts(
            experts = [expert_klass(dim = dim, mult = expert_mult, dropout = dropout) for _ in range(num_experts)],
            is_distributed = is_distributed,
            offload_unused_experts_to_cpu = offload_unused_experts_to_cpu
    ...
    def forward(self, x, mask = None, add_noise = False, noise_mult = 1.):
        ...
        # pdb.set_trace()
        combine_weights = rearrange(logits, 'b n e s -> b n (e s)')
        combine_weights = self.out_logit_transform(combine_weights)
        ...
        return share_out + out


if __name__ == "__main__":

    data = torch.randn((2, 8, 512))
    model = SS_MoE(dim=512, num_experts = 6, seq_len=8, num_slots = 4)
    output = model(data)
    print(output.shape)
