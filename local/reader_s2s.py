    ...
    def get_label(self, oracle_rttm):
        ...
        for line in files:
            ...
            if not session in MAX_len.keys():
                MAX_len[session] = 0
            #print(line)
            # print(line[3], '哈哈哈哈哈哈哈哈哈哈哈哈')
            start = np.int(float(line[3]) * 100)
            end = np.int(float(line[4]) * 100) + start
        ...
        for line in files:
            ...
            if not spk in rttm[session].keys():
                if self.differ_silence_inference_speech: #no sense
                    #rttm[session][spk] = np.zeros([MAX_len[session], 3], dtype=np.int8)
                    raise "differ_silence_inference_speech not implement"
                else:
    ...
    def get_mixture_utternce_label(self, session, speaker_embedding, start=0, end=None, check=True):
        ...
        for spk_idx in speaker_duration_id_order:
            ...
            if cur_num_speaker >= self.max_speaker:
                break
        return np.vstack(mixture_utternce_label).reshape(len(speakers), end - start)[sorted(range(len(speakers)), key=lambda k:speakers[k])], sorted(speakers)
